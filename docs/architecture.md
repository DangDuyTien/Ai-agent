...
- Agent soạn prompt gọi provider theo từng tác vụ, rồi ghép các prompt nhỏ thành `execution_prompt` để tránh gửi một yêu cầu quá lớn cho model yếu.
...
- Luồng phát triển tiếp append yêu cầu mới vào `rawIdea` bằng marker `Yêu cầu phát triển tiếp`, giữ bối cảnh/codebase cũ và chạy lại pipeline để lập kế hoạch cho phần bổ sung.
...
Dashboard có thể nhận upload thư mục dự án qua `POST /api/uploads/codebase`, copy các file hợp lệ vào `AI_AGENT_WORKSPACE_ROOT/uploads/:uploadId` hoặc `workspaces/uploads/:uploadId` mặc định, rồi tự dùng đường dẫn đó làm `sourcePath`. Luồng này bỏ qua `.git`, `node_modules`, `.next`, `dist`, `build`, `coverage` để tránh kéo theo dữ liệu nặng.
